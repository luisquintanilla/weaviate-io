...
One way to generate these vectors is to use a ["vectorizer" module](/weaviate/modules/retriever-vectorizer-modules/index.md), which will generate a vector at import and query time. This can be a convenient method if you are using a publicly available model and want to simplify your pipeline.
...
Weaviate is open source and you can easily [run the binary on Linux](/weaviate/installation/embedded.md) or [run a local instance from a Docker image](/weaviate/installation/docker-compose.md) on any OS, or even easier - you can create a free sandbox instance with the [Weaviate Cloud Services](/wcs/quickstart.mdx) (WCS).
...
Weaviate [stores data](/weaviate/concepts/data.md) objects in collections, each of which is called a `class`. Each object has a set of properties, and a vector representation generated automatically by a vectorizer, or specified at import time.
...
In this example, we've used the OpenAI API to generate it, with the same [`text-embedding-ada-002` model](https://platform.openai.com/docs/guides/embeddings/what-are-embeddings). Then, in the following query, we pass that vector to the [`nearVector` operator](/weaviate/api/graphql/search-operators.md#nearvector):
