...
One way to generate these vectors is to use a ["vectorizer" module](/developers/weaviate/modules/retriever-vectorizer-modules/index.md), which will generate a vector at import and query time. This can be a convenient method if you are using a publicly available model and want to simplify your pipeline.
...
Weaviate is open source and you can easily [run the binary on Linux](/developers/weaviate/installation/embedded.md) or [run a local instance from a Docker image](/developers/weaviate/installation/docker-compose.md) on any OS, or even easier - you can create a free sandbox instance with the [Weaviate Cloud Services](/wcs/quickstart.mdx) (WCS).
...
Weaviate [stores data](/developers/weaviate/concepts/data.md) objects in collections, each of which is called a `class`. Each object has a set of properties, and a vector representation generated automatically by a vectorizer, or specified at import time.
